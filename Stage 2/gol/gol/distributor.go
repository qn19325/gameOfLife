...
type distributorChannels struct {
	...
	keyPressFromController chan string
}

func handleController(client net.Conn, c distributorChannels) {
	reader := bufio.NewReader(client)
	for {
		keyPress, _ := reader.ReadString('\n')
		c.keyPressFromController <- keyPress[0:1]
	}
}
...
func distributor(p Params, c distributorChannels, controller net.Conn) {
	...
	for turns := 0; turns < p.Turns; turns++ {
		select {
		case pressed := <-c.keyPressFromController:
			if pressed == "s" {
				...
				for {
					tempKey := <-c.keyPressFromController
					if tempKey == "p" {
		...
		case <-ticker.C:
			...
			fmt.Fprintln(controller, "AliveCells: "+strconv.Itoa(aliveCellsNum))
		default:
		}
